defmodule Phoenix.Sync.Plug do
  ...
  `Phoenix.Sync.Plug` specifying a particular shape:

      defmodule MyAppWeb.Router do
        scope "/shapes" do
          pipe_through :browser

          get "/todos", Phoenix.Sync.Plug,
  ...
  logged in user id, then you can set an `authenticator` directly in the `Phoenix.Sync.Plug` configuration.
  ...
  def init(opts) do
    ...
    # runtime.
    %{client: Keyword.get(opts, :client, &Phoenix.Sync.client!/0)}
  ...
  you can just pass the name, so:
      ...
      Phoenix.Sync.Plug.shape!(Table, [:visible])
  ...
  If you need to match on something other than `==` then you can pass the operator in the params:
      ...
      Phoenix.Sync.Plug.shape!(Table, size: [>=: :size])
  ...
  """
  @spec shape!(Phoenix.Sync.shape_definition(), dynamic_shape_params()) :: term()
  ...
  ## Example
      ...
      get "/my-shapes/messages" do
        ...
        Phoenix.Sync.Plug.send_configuration(conn, shape)
      end
